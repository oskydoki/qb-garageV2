...
-- Transfer Vehicle Event (moved from RegisterNUICallback)
RegisterNetEvent('qb-garages:server:transferVehicle', function(data)
    ...
    local transferFee = data.transferFee
    
    if not Player then return end
    
    local TargetPlayer = QBCore.Functions.GetPlayer(targetPlayerId)
    if not TargetPlayer then
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.player_offline'), 'error')
        return
    end
    
    -- Check if player owns the vehicle
    local isOwned = MySQL.scalar.await('SELECT citizenid FROM player_vehicles WHERE plate = ? AND citizenid = ?', 
        { vehicleData.plate, Player.PlayerData.citizenid })
    
    if not isOwned then
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_vehicle_owner'), 'error')
        return
    end
    
    -- Get real vehicle price from QBCore.Shared.Vehicles
    local realVehiclePrice = 0
    if QBCore.Shared.Vehicles[vehicleData.vehicle] then
        realVehiclePrice = QBCore.Shared.Vehicles[vehicleData.vehicle].price or 0
    end
    
    -- Calculate transfer fee based on real price (2%)
    local realTransferFee = math.floor(realVehiclePrice * 0.02)
    
    -- Check if player has enough money for transfer fee
    local playerMoney = Player.PlayerData.money['bank']
    if playerMoney < realTransferFee then
    ...
    MySQL.update('UPDATE player_vehicles SET citizenid = ?, license = ? WHERE plate = ?', 
        { TargetPlayer.PlayerData.citizenid, TargetPlayer.PlayerData.license, vehicleData.plate })
    
    -- Charge transfer fee
    Player.Functions.RemoveMoney('bank', realTransferFee, 'vehicle-transfer-fee')
    
    -- Notify both players
    TriggerClientEvent('QBCore:Notify', src, 
        Lang:t('success.vehicle_transferred', 
            TargetPlayer.PlayerData.charinfo.firstname, 
            TargetPlayer.PlayerData.charinfo.lastname, 
            realTransferFee), 'success')
    
    TriggerClientEvent('QBCore:Notify', targetPlayerId, 
        Lang:t('success.vehicle_received',
            vehicleData.plate,
            Player.PlayerData.charinfo.firstname, 
            Player.PlayerData.charinfo.lastname), 'success')
end)
