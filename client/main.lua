...
local function OpenGarageMenu(data)
    QBCore.Functions.TriggerCallback('qb-garages:server:GetGarageVehicles', function(result)
        ...
        for _, v in pairs(result) do
            ...
            -- Get vehicle name and price from QBCore.Shared.Vehicles
            pcall(function()
                local vehicleData = QBCore.Shared.Vehicles[v.vehicle]
                if vehicleData then
                    vname = vehicleData.name
                    vprice = vehicleData.price or 0
                end
            end)
            ...
            formattedVehicles[#formattedVehicles + 1] = {
                ...
                price = vprice -- Add the real vehicle price
            }
        end
        ...
        local translations = {
            transfer = {
                title = Lang:t('transfer.title'),
                subtitle = Lang:t('transfer.subtitle'),
                player_id_label = Lang:t('transfer.player_id_label'),
                player_id_placeholder = Lang:t('transfer.player_id_placeholder'),
                fee_text = Lang:t('transfer.fee_text'),
                btn_cancel = Lang:t('transfer.btn_cancel'),
                btn_transfer = Lang:t('transfer.btn_transfer'),
                price_label = Lang:t('transfer.price_label')
            }
        }
...
local function CreateBlipsZones()
    for index, garage in pairs(Config.Garages) do
        ...
        local zone = nil
        if garage.type == 'gang' and garage.job == PlayerGang then
            zone = CreateZone(index, garage, 'gang')
        elseif garage.type == 'job' and garage.job == PlayerJob then
...
local function CheckPlate(vehicle, plateToSet)
    ...
    CreateThread(function()
        while true do
            Wait(500)
            local vehicleEntity = GetVehiclePedIsUsing(PlayerPedId())
